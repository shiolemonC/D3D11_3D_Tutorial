...
void BossModelSkinned_SetWorldMatrix(const XMMATRIX& world) { gWorld = world; }
void BossModelSkinned_SetLoop(bool loop) { gLoop = loop; }
void BossModelSkinned_SetPlaybackRate(float rate) { gPlayback = rate; }
void BossModelSkinned_Seek(float t) { gTime = t; }
bool BossModelSkinned_LoadAnimOnly(const std::wstring& p) { return LoadAnim(p); }
...
bool BossModelSkinned_SampleRootDelta_Local(float dt, XMFLOAT3* outDeltaT) {
    if (!outDeltaT) return false;
    outDeltaT->x = outDeltaT->y = outDeltaT->z = 0.0f;

    if (gFrameCount == 0 || gJoints.empty()) return false;
    int root = BossModelSkinned_GetMotionRootIndex();
    if (root < 0) root = BossModelSkinned_GetRootJointIndex();
...
// —— 入场对齐 —— 
void BossModelSkinned_SetRootYawAlignTarget(float yawTargetRad) { gRootYawAlignTarget = yawTargetRad; gRootYawAlignEnabled = true; }
void BossModelSkinned_ResetRootYawTrack(float yawStartRad) { gRootYawStart = yawStartRad; }

// —— Node 层修正 —— 
void  BossModelSkinned_SetNodeYawFix(float r) { gNodeYawFixRad = r; }
float BossModelSkinned_GetNodeYawFix() { return gNodeYawFixRad; }
...
uint32_t BossModelSkinned_GetFrameCount() { return gFrameCount; }
float    BossModelSkinned_GetSampleRate() { return gSampleRate; }
...
bool  BossModelSkinned_SetMotionRootByName(const char* utf8Name) {
    ...
    if (!gJoints.empty()) BossModelSkinned_ResolveMotionRoot();
    return true;
}
int   BossModelSkinned_GetMotionRootIndex() {
