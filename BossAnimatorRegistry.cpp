...
static void ApplyWorldWithRootMotion()
{
    ...
    BossModelSkinned_SetWorldMatrix(T * gBaseWorld);
}
...
bool BossAnimatorRegistry_Initialize(ID3D11Device* dev, ID3D11DeviceContext* ctx)
{
    ...
    return BossModelSkinned_Initialize(dev, ctx);
}
...
bool BossAnimatorRegistry_Has(const std::wstring& name)
{
    return FindIndex(name) >= 0;
}

const AnimClipDesc* BossAnimatorRegistry_Get(const std::wstring& name)
{
    ...
    bool overrideRate, float rateValue)
{
    ...
    if (!clip.motionRootNameUTF8.empty()) {
        BossModelSkinned_SetMotionRootByName(clip.motionRootNameUTF8.c_str());
    ...
    // —— 入场对齐（基于 motion-root）——
    {
        ...
        float yaw0 = 0.0f;
        if (BossModelSkinned_DebugGetRootYaw_F0(&yaw0)) {
            ...
            if (BossModelSkinned_ComputeRootYaw_ModelSpace_FirstFrame(&yawModel0)) {
                ...
                BossModelSkinned_SetNodeYawFix(nodeFix);
    ...
    {
        float yaw0 = 0.0f;
        BossModelSkinned_DebugGetRootYaw_F0(&yaw0);
    ...
    BossModelSkinned_SetLoop(loop);
    BossModelSkinned_SetPlaybackRate(rate);
    ...
    BossModelSkinned_SetWorldMatrix(gBaseWorld);
    return true;
}

void BossAnimatorRegistry_SetWorld(const XMMATRIX& world)
{
    gBaseWorld = world;
...
void BossAnimatorRegistry_Update(double dtSec)
{
    ...
    case RootMotionType::UseAnimDelta:
    {
        ...
        float dyaw = 0.0f;
        if (BossModelSkinned_SampleRootYawDelta((float)dtSec, &dyaw)) {
    ...
    if (gBlend.active) {
        ...
        BossModelSkinned_SetCrossFadeWeight(w);
    }
    else {
...
{
    BossModelSkinned_Draw(); // 里面会把 node-fix 乘到 World 上
}
...
bool BossAnimatorRegistry_DebugGetRootYaw(float* yaw0, float* yawNow)
{
    ...
    float a = 0.0f, b = 0.0f;
    bool ok0 = BossModelSkinned_DebugGetRootYaw_F0(&a);
    bool ok1 = BossModelSkinned_DebugGetRootYaw_Current(&b);
...
bool BossAnimatorRegistry_DebugGetCurrentClipLengthSec(float* outSec)
{
    ...
    const uint32_t fc = BossModelSkinned_GetFrameCount();
    const float    sr = BossModelSkinned_GetSampleRate();
    ...
    const char* curveNameUTF8)
{
    ...
    if (durationSec <= 0.0f) {
        gBlend.active = false;
        BossModelSkinned_ClearCrossFadeSource();
        BossAnimatorRegistry_Play(name, nullptr);
        return;
    }

    // 1) 让底层记录“当前姿势快照”
    BossModelSkinned_CaptureCurrentLocalPose();

    // 2) 播放新剪辑（沿用原有 Play 逻辑：加载 .mesh/.skel/.anim，设置 rmType 等）
    bool changed = false;
    if (!BossAnimatorRegistry_Play(name, &changed)) {
        // 播放失败则取消混合
        gBlend.active = false;
        BossModelSkinned_ClearCrossFadeSource();
        return;
